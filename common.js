  ...
  function renderRevealMedia(media, fallbackText) {
    ...
    if (item.mode === 'image' && item.imageUrl) {
      return '<div class="reveal-media media-image">' +
        '<div class="reveal-media-frame">' +
          '<img src="' + escapeHtml(item.imageUrl) + '" alt="' + escapeHtml(item.alt || fallbackText || 'Related image') + '"' + (item.objectPosition ? ' style="object-position: ' + escapeHtml(item.objectPosition) + ' !important"' : '') + ' loading="eager" referrerpolicy="no-referrer" onerror="this.parentNode.parentNode.classList.add(&quot;is-broken&quot;); this.remove();">' +
